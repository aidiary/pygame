...
def main():
    ...
    pygame.display.set_caption(u"PyRPG 21 デバッグ情報の表示")
    # サウンドをロード
...
def show_info(screen, msg_engine, player, map):
    """デバッグ情報を表示"""
    msg_engine.draw_string(screen, (10,10), map.name.upper())  # マップ名
    msg_engine.draw_string(screen, (10,40), player.name.upper())  # プレイヤー名
    msg_engine.draw_string(screen, (10,70), "%d_%d" % (player.x, player.y))  # プレイヤー座標
        ...
        if self.moving == True:
            ...
            if self.rect.left % GS == 0 and self.rect.top % GS == 0:  # マスにおさまったら移動完了
                ...
                if isinstance(event, MoveEvent):  # MoveEventなら
                    ...
                    dest_x = event.dest_x
                    dest_y = event.dest_y
                    map.create(dest_map)  # 移動先のマップへ移動
